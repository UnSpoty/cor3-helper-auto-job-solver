...
(function () {
    ...
    async function render(container) {
        container.innerHTML = '';
        container.appendChild(el('div', 'section-title', 'Logs'));
        ...
        toolbar.innerHTML = `
            <select id="log-mod"><option value="">all modules</option>${moduleIds.map((m) => `<option value="${m}">${m}</option>`).join('')}</select>
            <select id="log-lvl">
            ...
            <button class="btn btn-danger small" id="log-clear">Clear</button>
        `;
