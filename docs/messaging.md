...
| `WS.MERC_CONFIGURE` | `COR3_WS_MERC_CONFIGURE` | `{ mercenaryId, data: { totalCost, riskScore, insuranceCost, … }, insured }` | per-merc cost/risk. Multiple events for one fetch (one per merc). `insured` = the `hasInsurance` flag the preview was REQUESTED with (the reply doesn't echo it) — merc-config persists it as `_insured`. Also emitted by a launch's own configure ack (correctly attributed to the launched merc). |
...
| `WS.EXPEDITION_LAUNCH_ERROR` | `COR3_WS_EXPEDITION_LAUNCH_ERROR` | `{ error, retryAfter? }` | `'Maximum 1 active expedition allowed'` carries `retryAfter:120000` and schedules `WS.EXPEDITION_RETRY_LAUNCH`; `'configure-timeout'` = the launch's configure ack was dropped so the launch was ABORTED; any other server rejection of `launch` is surfaced here too (never a silent no-op). auto-send just unlocks and retries next cycle. |
...
| `GAME.LAUNCH_EXPEDITION` | `COR3_LAUNCH_EXPEDITION` | `{ config: {mercenaryId, marketId, locationConfigId, zoneConfigId, goalId, hasInsurance} }` | `expeditions.configure` → configure ACK → `expeditions.launch`, **serialized on the configure chain**: no cost-preview configure can interleave between the two frames (an interleaved preview clobbered the configure state and launched insured picks UNINSURED), and the ack re-prices the launched merc (`WS.MERC_CONFIGURE`). A dropped ack aborts with `WS.EXPEDITION_LAUNCH_ERROR {error:'configure-timeout'}`. The DTO field is `goalId` (post-patch — the server rejects `objectiveId`: "property objectiveId should not exist; goalId must be a string"). |
...
| `AUTOJOBS.COMPLETE_JOB` | `ajCompleteJob` | popup → isolated (runtime) | `{ jobId, marketId }` | Jobs list "✓" on a READY-to-complete row (TAKEN job the game flags `canComplete`). Orchestrator posts `MSG.GAME.COMPLETE_JOB` — **refused while the loop runs** (same endpoint-flip hazard as DISMISS_FAILED; the READY_TO_COMPLETE step claims it instead). |
...
| `AUTOJOBS.HACK_TRANSIT` | `COR3_AJ_HACK_TRANSIT` | isolated → MAIN (window) | `{ gateServerId, gateServerType, gateServerName }` | route-opening (`hackTransitNodes`): hack a non-public/no-access transit GATE node so the game relays THROUGH it. The bridge connects + hacks the gate (`ensureHack` vs its LIVE `serverDefenceRate`); **no** SAI login after — only the access grant is needed. |
| `AUTOJOBS.HACK_RESULT` | `COR3_AJ_HACK_RESULT` | MAIN → isolated (window) | `{ gateServerName, success, retryable, reason }` | gate-hack outcome (`_openRoute` parks on it). `success` → route open; `retryable:false` → PERMANENTLY un-openable (`planHack` none/underpower) → orchestrator records it in `AJ_BUGGED_GATES`; `retryable:true` → retried up to `MAX_FLOW_ATTEMPTS`. |
...
| `VALUABLE.SELL_ACTION` | `vsSell` | popup → isolated (runtime) | `{ serverIds:[] }` | download + sell for the selected servers (empty list = just sell what's already in Downloads). Same guards as SCAN_ACTION. |
...
| `VALUABLE.SELL_START` | `COR3_VS_SELL_START` | isolated → MAIN (window) | `{ servers:[…] }` | sell run: per server `ensureAccess` → `ensureSearch` (maximize-power SEARCH loadout) → `file/log.search-valuable` → re-read lists, filter to detected → `file/log.download`; then per market (USOL → SRM → DARK → HOME) `get.sellable-items` → `sell.items` one by one. |
