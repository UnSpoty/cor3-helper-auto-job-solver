...
| `CHECK_ACCESS` | `checkAccess` | per server: `accessible` / `hasSaiAccess` / `onCooldown` (the last from the graph's `isInMaintenance` flag) / `noPath`. `noPath` comes from a BFS over `connections[]` from HOME (`computePathReachability`): a maintenance server can be a path's ENDPOINT but never a transit hop, so a K/D'd node cuts off every server behind it (A-B-C-D: B on maintenance → C and D get `noPath`). Does **not** decide market reachability — that is the OUTPUT of UPDATE_MARKETS' refresh probe (see below). |
...
| `CHECK_CONDITION` | `checkCondition` | per job, eligibility + explicit `skipReason`. Wired conditions: bugged registry; and (only when the job has a server) server-known / K-D cooldown / no-path (route from HOME blocked by a maintenance transit node) / accessible / user-SKIP / type-disabled (`AJ_SERVER_OVERRIDES`). A missing related server is **not** a skip reason. |
